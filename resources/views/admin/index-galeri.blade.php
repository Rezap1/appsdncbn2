...
<head>
    ...
    <title>Museum Visual | SDN Cibinong 2</title>
    ...
    <style>
        ...
        /* Tombol Dashboard - Mudah Dikenali */
        .btn-dashboard {
            background: #111;
    ...
    <div class="max-w-7xl mx-auto">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
            <div class="space-y-2">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-amber-600 text-[10px] font-black uppercase tracking-[0.4em]">Kelola Galeri</span>
                </div>
                ...
                <p class="text-slate-500 font-medium text-sm max-w-md leading-relaxed mt-4">
                    Menampilkan dokumentasi sejarah dan momen berharga <span class="text-slate-300 font-bold border-b border-amber-900">SDN Cibinong 2</span>.
                </p>
            </div>
            ...
            <div class="flex flex-wrap gap-4">
                ...
                <a href="{{ route('admin.galeri.create') }}" class="btn-add flex items-center gap-2 uppercase">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            ...
            <div class="museum-card overflow-hidden group">
                <div class="relative h-72 overflow-hidden bg-zinc-900">
                    <img src="{{ asset('uploads/galeri/'.$g->gambar) }}" class="w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-all duration-700 group-hover:scale-110">

                    <div class="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center">
                        <form action="{{ route('admin.galeri.destroy', $g->id) }}" method="POST" onsubmit="return confirm('Hapus karya ini dari galeri?')">
                            @csrf @method('DELETE')
                    ...
                    <div class="absolute top-4 left-4">
                        <span class="bg-black/60 backdrop-blur-md border border-white/10 text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-md text-amber-500">
                            Exhibit #{{ $loop->iteration }}
                        </span>
                    </div>
                </div>

                <div class="p-6 border-t border-white/5">
                    <h3 class="font-bold text-sm text-zinc-100 mb-2 truncate tracking-wide">{{ $g->judul }}</h3>
            ...
            <div class="col-span-full py-32 text-center empty-state rounded-[40px]">
                ...
                <h3 class="text-xl font-bold text-zinc-500 tracking-widest uppercase">Gudang Galeri Kosong</h3>
                <p class="text-zinc-600 mt-3 text-xs font-medium uppercase tracking-tighter">Unggah Disini.</p>
                <a href="{{ route('admin.galeri.create') }}" class="mt-10 inline-block px-10 py-3 border border-zinc-800 text-zinc-400 font-bold text-[10px] uppercase tracking-widest rounded-xl hover:border-amber-600 hover:text-amber-500 transition-all">
                    Tambah Galeri Baru
                </a>
            </div>
            @endforelse
        ...
        <footer class="mt-32 mb-10 text-center opacity-20">
            <p class="text-[8px] font-black tracking-[1em] text-zinc-400 uppercase italic">Established 2026 • Digital Library SDN 2</p>
        </footer>
    </div>
</body>
</html>
